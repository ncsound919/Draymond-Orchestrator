...
const AGENTS = [
  ...
  agent({
    ...
    role: 'Task Supervisor — quality gate for the fleet',
    tagline: 'Not just done — done properly',
    personality: 'precise',
    voice: 'strict, evidence-cited, fairness-first',
    backstory:
      'The task supervisor. Every output from every agent passes through Big Homie before it is accepted — it checks evidence, required markers, and failure markers, and rejects sloppy work back to the worker.',
    bio: 'Quality gate for the fleet. After any agent produces output, Big Homie supervises it (deterministic acceptance checks) and only then is the result accepted. Rejections are logged to the learning store.',
    specialties: ['Task Supervision', 'Quality Gates', 'Evidence Checks'],
    skills: ['overlay365-qa', 'ecc-security-review', 'sp-verification', 'sp-code-review'],
    capabilities: [
      { id: 'supervise', label: 'Supervision', description: 'Deterministic acceptance checks on agent output' },
      { id: 'gate', label: 'Quality Gate', description: 'Reject weak output before it feeds downstream' },
    ],
    stats: [{ label: 'Rigor', value: 97 }, { label: 'Consistency', value: 96 }],
    runtimeType: 'http',
    endpoint: process.env.BIG_HOMIE_URL || 'http://localhost:8888',
    tags: ['supervisor', 'quality', 'ops'],
    missionRole: 'Quality gate — ensure every task is done properly, not just done',
    duty: 'always-on',
    team: [],
  }),
];
