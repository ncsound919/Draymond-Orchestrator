...
const jobs = db
  ...
  .all();
const ents = db
  .prepare("SELECT slug, invocation_method FROM draymond_entities WHERE slug IN ('uplift-agent','grader','reporank','mutly','omni-research','overlay-marketing-actions','kaggle')")
  .all();
const steps = db
