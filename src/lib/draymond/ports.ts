...
export const TOOL_PORTS: ToolPort[] = [
  ...
  // -- Review & grading -----------------------------------------------------
  { slug: 'reporank', name: 'RepoRank', category: 'review', port: 3200, env: 'REPORANK_URL', health: '/health', cwd: 'agents/reporank', start: 'pnpm dev:local', notes: 'Repo depth scoring + remediation + milestones/gates/drift progress. Health route is GET /health (not /api/health). Moved off 3001 (now the OSS Twenty CRM).' },
  ...
  { slug: 'codenexus', name: 'CodeNexus', category: 'review', port: 3205, env: 'CODENEXUS_URL', health: '/health', cwd: 'agents/CodeNexus-main/control-plane', start: 'npx tsx src/node-adapter.ts', notes: 'Agentic PR review + fix platform (webhook → diff → security scan → comment → auto-fix → verify → push). Health route is /health (node-adapter only serves /health, not /api/health). Runs from source via tsx (control-plane tsconfig maps @codenexus/* to package src). Deterministic deep-audit lenses run credential-free via scripts/local-codenexus.ts in the benchmark loop.' },
  ...
  { slug: 'dev-brain', name: 'Dev Brain', category: 'orchestration', port: 3450, env: 'DEV_BRAIN_URL', health: '/api/health', cwd: 'Dev-Brain', start: 'node dist/server.cjs', notes: 'Deterministic multi-agent reasoning engine (FSM candidate-triage + decision trees, no LLM required; Gemini optional). Drives the weekly github-awesome tool intake scan (POST /api/intake).' },
  { slug: 'halofy', name: 'Halofy (HaloMem kernel)', category: 'security', port: 8787, env: 'HALOMEM_URL', health: '/health', cwd: '04_Integrations/github-awesome/halofy/kernel', start: 'npm run serve', notes: 'MCP/HTTP governance boundary — server-side identity/role/namespace ACL, provenance, append-only audit, signed erasure. Embedded PGlite local default (zero-setup). AGPL-3.0. Kernel validated via `npm run demo`; HTTP serve bootstrap needs model/embedder env config.' },
  { slug: 'eidos', name: 'Eidos', category: 'service', port: 8420, env: 'EIDOS_URL', health: '/', cwd: '04_Integrations/github-awesome/eidos', start: 'eidos serve <file> --port 8420', notes: 'Single-file relational spreadsheet for you + your agent (AGPL-3.0). CLI creates/serves .eidos files (agent-editable SQLite). Confirmed working on :8420.' },
  ...
  { slug: 'omniresearch-pro', name: 'OmniResearch', category: 'service', port: 3012, env: 'OMNI_RESEARCH_URL', health: '/api/health', cwd: 'agents/omniresearch 2', start: 'npx tsx server.ts', notes: 'Deep research agent (OmniResearch 2) — replaced the legacy OmniResearch-Pro-main (retired 2026-09-23). Models route through LiteLLM (:4100); decision/synergy/Jev/governance via Dev-Brain (:3450); verify/repair/provenance via Recourse (:3050); repairs dispatched to Axiom (:3198) via OpenHub (:3010). Port 3012 (3010 is the OpenHub console).' },
  ...
  { slug: 'open-seo', name: 'OpenSEO', category: 'service', port: 3006, env: 'OPEN_SEO_URL', health: '/api/health', notes: 'Open-source SEO engine (MIT) — keyword research, rank tracking, backlinks, site audits, AI visibility. Needs a DataForSEO key (Keywire); GSC needs BETTER_AUTH_SECRET. Managed via agents/open-seo/compose.yaml. Moved off 3001 (OSS Twenty CRM).' },
  ...
  { slug: 'oss-listmonk', name: 'Listmonk (email)', category: 'service', port: 9002, env: 'LISTMONK_URL', health: '/', notes: 'Self-hosted newsletter/email (AGPL). Managed via oss-marketing-stack/listmonk.yml.' },
  { slug: 'oss-twenty', name: 'Twenty (CRM)', category: 'service', port: 3001, env: 'TWENTY_URL', health: '/healthz', notes: 'Open-source CRM (AGPL). Managed via oss-marketing-stack/twenty.yml.' },
  { slug: 'oss-formbricks', name: 'Formbricks (lead capture)', category: 'service', port: 3002, env: 'FORMBRICKS_URL', health: '/health', notes: 'Open-source survey/lead capture (AGPL). Managed via oss-marketing-stack/formbricks/docker-compose.yml.' },
  { slug: 'oss-umami', name: 'Umami (analytics)', category: 'service', port: 3003, env: 'UMAMI_URL', health: '/', notes: 'Open-source web/product analytics (MIT) — event API feeds strategist trend/anomaly intel. Managed via oss-marketing-stack/umami.yml.' },
  { slug: 'oss-windmill', name: 'Windmill (automation)', category: 'service', port: 8001, env: 'WINDMILL_URL', health: '/api/version', notes: 'Open-source scripts-as-APIs automation (AGPL) — orchestration spine for unified-publish + strategy. Managed via oss-marketing-stack/windmill.yml. Host port 8001 (8000 is Uplift Agent).' },
  ...
  { slug: 'sub-team', name: 'Sub Team', category: 'service', port: 8050, env: 'SUB_TEAM_URL', health: '/health', cwd: 'agents/Sub-Team-main', start: 'python main.py --serve --port 8050', notes: 'Deterministic CPU RTL pipeline + CrewAI workforce + cross-disciplinary/business analysis. Endpoints: /execute, /pipeline/cpu, /pipeline/analyze, /pipeline/business, /memory.' },
  { slug: 'lil-homie', name: 'Lil Homie', category: 'service', port: 18790, env: 'NANOBOT_URL', health: '/', cwd: 'agents/lil-homie', start: 'nanobot gateway', notes: 'Nanobot multi-channel chat/automation gateway (default port 18790). Needs a nanobot config with a provider key + channel login before it will boot. Registered as a known tool, not an always-on pm2 service.' },
