...
export const TOOL_PORTS: ToolPort[] = [
  ...
  // ── Knowledge graph ──────────────────────────────────────────────────────
  { slug: 'graphify', name: 'Graphify', category: 'knowledge', port: 3203, env: 'GRAPHIFY_URL', health: '/health', cwd: 'graphify-out', start: 'python -m graphify.serve graphify-out/graph.json --transport http --port 3203', notes: 'Codebase knowledge graph (tree-sitter AST, no vector store). Query via MCP: query_graph / shortest_path / explain. Index with: scripts/graphify-index.ps1.' },
